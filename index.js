...
class Room {
  constructor(name, area) {
    this.name = name;
    this.area = area;
    this._id = Date.now(); // Generate unique id for each room
  }
}
...
class HouseService {
  ...
  static createHouse(name) {
    return $.post(this.url, { name: name });
  }
...
class DOMManager {
  ...
  static addRoom(id) {
    for (let house of this.houses) {
      if (house._id == id) {
        const roomName = $(`#${house._id}-room-name`).val();
        const roomArea = $(`#${house._id}-room-area`).val();
